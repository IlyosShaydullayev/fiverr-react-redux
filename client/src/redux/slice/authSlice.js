...
import { handleRegisterService, handleLoginService, userUpdateInfo } from "../service/authService";
...
export const handleLogin = createAsyncThunk("auth/login", async(data, thunkApi) => {
  try {
    const { data: {user, jwt}} = await handleLoginService(data)
    data.setCookies('token', jwt)
    return user
  } catch (error) {
    return thunkApi.rejectWithValue(error.response.data.message || error);
  }
...
export const handleSetUserInfo = createAsyncThunk("auth/setUserInfo", async(data, thunkApi) => {
  try {
    const { data: userData } = await userUpdateInfo(data)
    return userData
  } catch (error) {
    return thunkApi.rejectWithValue(error.response.data.message || error);
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(handleSetUserInfo.fulfilled, (state, action) => {
        ...
        state.userInfo = {...state.userInfo, image: action.payload.length ? action.payload : null}
      })
